...
class CSystem:
    def __init__(self):
        ...
        lidarBlindAngDeg = rospy.get_param("~lidar_blind_angle", default = 45)
        lidarBlindAngWidthDeg = rospy.get_param("~lidar_blind_angle_width", default = 5)
        self.__tfMan = CTfManager()
        self.__movCtrl = CMovementController()
        self.__gps = CGpsSystem(self.__movCtrl, 1.0 / rate)
        self.__lidar = CLidar(lidarFovDeg, lidarEffectiveFovDeg, lidarClasterSize, lidarBlindAngDeg, lidarBlindAngWidthDeg)
